...
public class SimilaritySearchImpl implements SimilaritySearch {

    private Document original;
    private Document diff;

    public SimilaritySearchImpl(Document original, Document source) {
    ...
    public Element findSimilar(String id) {
        Element origin = getOrigin(id);
        return findForId(id).orElseGet(() -> findSimilarByAttrs(origin));
    }

    @Override
    public Element getOrigin(String id) {
